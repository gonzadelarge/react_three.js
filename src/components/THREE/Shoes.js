...
export const ShoeApp = () => {

    const { range } = useControls( { range: { value: 100, min: 1, max: 1000, step: 10 } } );
...
function Shoe({ random, ...props }) {
    ...
    useFrame((state) => {
        ...
        ref.current.position.y = Math.sin(t / 1.2) / 3
        ref.current.scale.x = ref.current.scale.y = ref.current.scale.z = THREE.MathUtils.lerp(ref.current.scale.z, hovered ? 1.8 : 1, 0.1)
        ref.current.color.lerp(color.set(hovered ? 'crimson' : 'white'), hovered ? 1 : 0.1)
    })
    return (
        <group {...props}>
            <Instance
